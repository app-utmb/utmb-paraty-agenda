...
import { MapaExpo } from "./MapaExpo";

type Visao = "marcas" | "mapa";
...
/** Estandes de marca em ordem alfabetica. Areas de servico ficam so no mapa. */
export const MARCAS_DA_EXPO = PONTOS_MAPA.filter(
  (p) => p.tipo === "marca",
).sort((a, b) =>
...
function CartaoMarca({
  ...
  return (
    <button type="button" className="marca-tile" onClick={() => aoAbrir(ponto)}>
      ...
      {temBeneficio && (
        <span className="marca-tile__tag">{rotuloBeneficio}</span>
      )}
    </button>
  );
}
...
/**
 * Aba Ativacoes: junta os benefícios e o mapa. Abre numa grade com o logo de
 * cada marca, porque o atleta reconhece a marca antes de ler o nome, e cada
 * logo abre o mesmo detalhe do estande que o mapa ja mostrava.
 */
export function Ativacoes({ dados, aoAbrirPonto, aoAbrirBeneficio }: Props) {
  const { idioma, t } = useIdioma();
  const [visao, setVisao] = useState<Visao>("marcas");
  ...
  const comBeneficio = useMemo(
    () =>
      new Set(
        MARCAS_DA_EXPO.filter(
          (p) => beneficiosDaMarca(p, dados.beneficios).length > 0,
        ).map((p) => p.id),
      ),
    [dados.beneficios],
  );
  ...
  const semEstande = useMemo(() => {
    const ligados = new Set(
      PONTOS_MAPA.flatMap((p) =>
        beneficiosDaMarca(p, dados.beneficios).map((b) => b.id),
      ),
    );
  ...
  return (
    <div>
      ...
      <div className="alternador" role="group" aria-label={t.ativacoes.visao}>
        {(["marcas", "mapa"] as const).map((v) => (
          <button
            key={v}
            type="button"
            className="alternador__botao"
      ...
      {visao === "mapa" ? (
        <MapaExpo config={dados.config} aoAbrirPonto={aoAbrirPonto} />
      ) : (
        <>
